...
import {
  NO_PROJECT_SELECTED_MESSAGE,
  SPECIALTY_ERROR_MESSAGES,
  createSpecialty,
  formValues,
} from "@/lib/services/specialties";
import { parseSpecialtyInput } from "@/lib/validation/specialty";

export const prerender = false;

export const POST: APIRoute = async (context) => {
  const back = "/specialties";
  ...
  if (!selected.ok) {
    return context.redirect(errorUrl(back, SPECIALTY_ERROR_MESSAGES[selected.error], formValues(form)));
  }
  if (!selected.data) {
    return context.redirect(errorUrl(back, NO_PROJECT_SELECTED_MESSAGE, formValues(form)));
  }

  const result = await createSpecialty(supabase, selected.data, parsed.data);
  if (!result.ok) {
